  ...
  return (
    <View style={{ gap: 18 }}>
      ...
      <View style={{ borderRadius: 22, padding: 18, backgroundColor: colors.paper, borderWidth: 1, borderColor: colors.mist, gap: 10 }}>
        <Text style={{ fontSize: 15, color: colors.graphite, fontWeight: '700', fontFamily: 'Nunito-Bold' }}>
          {locale === 'nl-NL'
        ...
        {FAMILIAR_VOICE_STATUS.recorded ? (
          <View style={{ backgroundColor: colors.sagePale, borderRadius: 14, padding: 10 }}>
            <Text style={{ fontSize: 14, fontWeight: '800', fontFamily: 'Nunito-Bold', color: colors.sage }}>
          ...
          >
            <Text style={{ color: '#fff', fontSize: 16, fontWeight: '900', fontFamily: 'Nunito-Black' }}>
              {locale === 'nl-NL' ? 'Start opname' : 'Start recording'}
            </Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <View style={{ borderRadius: 22, padding: 18, backgroundColor: colors.paper, borderWidth: 1, borderColor: colors.mist, gap: 8 }}>
        {[
          ...
          <View key={item.label} style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingVertical: 4 }}>
            ...
            <Text style={{ fontSize: 15, fontWeight: '800', fontFamily: 'Nunito-Bold', color: colors.ink, flex: 1 }}>{item.label}</Text>
            <Text style={{ fontSize: 15, fontWeight: '700', fontFamily: 'Nunito-Bold', color: colors.graphite }}>{item.value}</Text>
          </View>
        ))}
      </View>
      ...
      <View style={{ borderRadius: 18, padding: 14, backgroundColor: colors.slatePale, gap: 6 }}>
        <Text style={{ fontSize: 14, fontWeight: '900', fontFamily: 'Nunito-Black', color: colors.slate }}>
          {locale === 'nl-NL' ? 'Uw gegevensrechten (AVG)' : 'Your data rights (GDPR)'}
        </Text>
        <Text style={{ fontSize: 13, color: colors.graphite, fontWeight: '600', fontFamily: 'Nunito-SemiBold' }}>
          {locale === 'nl-NL'
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginTop: 4 }}>
          <TouchableOpacity onPress={() => ctx.onPrimaryAction('DATA_EXPORT')} style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 12, backgroundColor: colors.paper }}>
            <Text style={{ fontSize: 13, fontWeight: '800', fontFamily: 'Nunito-Bold', color: colors.slate }}>{locale === 'nl-NL' ? 'Exporteer' : 'Export'}</Text>
          </TouchableOpacity>
          <TouchableOpacity onPress={() => ctx.onPrimaryAction('DATA_DELETE')} style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 12, backgroundColor: colors.rosePale }}>
            <Text style={{ fontSize: 13, fontWeight: '800', fontFamily: 'Nunito-Bold', color: colors.rose }}>{locale === 'nl-NL' ? 'Verwijder alles' : 'Delete all'}</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      >
        <Text style={{ color: colors.rose, fontSize: 16, fontWeight: '900', fontFamily: 'Nunito-Black' }}>
