      ...
      {visits.map((visit) => (
        <View key={visit.id ?? visit.carer + visit.date} style={{
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 17, fontWeight: '900', fontFamily: 'Nunito-Black', color: colors.ink }}>{visit.carer}</Text>
              <Text style={{ fontSize: 13, color: colors.pewter, fontWeight: '700', fontFamily: 'Nunito-Bold' }}>
          ...
          {visit.handover && visit.handover.appetite ? (
            <View style={{ borderRadius: 14, padding: 12, backgroundColor: colors.mist, gap: 4 }}>
              <Text style={{ fontSize: 13, fontWeight: '900', fontFamily: 'Nunito-Black', color: colors.ink }}>
                {locale === 'nl-NL' ? 'Overdracht' : 'Handover'}
              </Text>
              {Object.entries(visit.handover).filter(([, v]) => v).map(([key, val]) => (
                <Text key={key} style={{ fontSize: 12, color: colors.graphite, fontWeight: '600', fontFamily: 'Nunito-SemiBold' }}>
                  <Text style={{ fontWeight: '800', fontFamily: 'Nunito-Bold' }}>{key}: </Text>{val}
                </Text>
              ))}
            </View>
          ) : null}
          ...
          {visit.marEntries.length > 0 && (
            <View style={{ borderRadius: 14, padding: 12, backgroundColor: colors.sagePale, gap: 4 }}>
              <Text style={{ fontSize: 13, fontWeight: '900', fontFamily: 'Nunito-Black', color: colors.sage }}>MAR</Text>
              {visit.marEntries.map((entry, idx) => (
                <Text key={idx} style={{ fontSize: 12, color: colors.ink, fontWeight: '600', fontFamily: 'Nunito-SemiBold' }}>
